...
5. После завершения работы программы, консоль должна содержать информацию о том, что все лошади финишировали. Пример
    сообщения для первой лошади: "Horse_01 has finished the race!".
*/
...
public class Solution
{
    ...
    public static int calculateHorsesFinished(List<Horse> horses) throws InterruptedException
    {
        ...
        for (Horse h : horses)
        {
            if (!h.isFinished())
            {
                System.out.println("Waiting for " + h.getName().replace('_',' '));
                h.join();
            }
            else
            {
                System.out.println(h.getName() + " has finished the race!");
                countFinished++;
            }
        }
