...
4. В методе readObject(ObjectInputStream in) поле stream должно быть инициализировано новым объектом типа
    FileOutputStream с параметрами(fileName, true).
5. В конструкторе класса Solution поле stream должно быть инициализировано новым объектом типа FileOutputStream с
    параметром(fileName).
*/
public class Solution implements Serializable, AutoCloseable
{
    ...
    {
        Solution obj = new Solution("d:/i.txt");
        ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("d:/o.txt"));
        obj.writeObject("aaa");
        obj.writeObject(oos);
        oos.close();
        ObjectInputStream ois = new ObjectInputStream(new FileInputStream("d:/o.txt"));
